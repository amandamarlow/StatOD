function [X, P, y, alpha] = EKF_DMC(t, data, R, Qc, X0, P0, tau, warmStart, constants)
...
%     STM = eye(n);
    ...
    if warmStart
        ...
        numtw = length(twarm);
        dx0 = zeros(n,1);
        [Xwarm, dxwarm, P(:,:,1:numtw), y(:,1:numtw), alpha(:,1:numtw)] = CKF_DMC(twarm, data(1:numtw,:), R, Qc, X0, dx0, P0, tau, constants);
        X(:,1:nw) = Xwarm+dxwarm;
        tfirst = numtw+1;
    end
    j = 1;
    for i = tfirst:length(t)
        ...
        [Xvec, STMvec] = DMCintegrateTrajectorySTM_HW3([t(i-1), t(i)], X(:,i-1), eye(n), tau, constants);
        X(:,i) = Xvec(:,end);
        STM = STMvec(:,:,end);
