function [X, dx0, P, y, alpha, iterations, RMSresidual_post] = batch_HW4(data, R, X0, dx0, P0, max_iterations, constants)
%Batch filter
...
%     max_iterations = 3;
    for q = 1:max_iterations+1
        ...
        if min(data(:,1)) == t(1)
            Y = data(1,3:4)';
            stationNum = data(1,2);
            [GofXt0, Hfull] = GandH_HW3(t(1), X(:,1), stationNum, constants); % STM at t0 is I
            % if measurement_type == "range"
            %     Y = data(1,3);
            %     GofXt0 = GofXt0(1);
            %     H(:,:,1) = Hfull(1,:,1);
        ...
        [X, STM] = integrateTrajectorySTM_HW3(data(:,1), X(:,1), eye(n), constants);
