...
% term1 = -3 * ae^2 * J2 * mu / (2 * r^8 * sqrt(r^2));
% term2 = 15 * ae^2 * J2 * mu / (2 * r^8 * sqrt(r^2));
% 
% aJ2_E_partial_R_E = [
%     term1 * (r^4 + 35 * x^2 * z^2 - 5 * r^2 * (x^2 + z^2)), term2 * x * y * (r^2 - 7 * z^2), term2 * x * (3 * r^2 * z - 7 * z^3);
%     term2 * x * y * (r^2 - 7 * z^2), term1 * (r^4 + 35 * y^2 * z^2 - 5 * r^2 * (y^2 + z^2)), term2 * y * (3 * r^2 * z - 7 * z^3);
%     term2 * x * (3 * r^2 * z - 7 * z^3), term2 * y * (3 * r^2 * z - 7 * z^3), term1 * (3 * r^4 - 30 * r^2 * z^2 + 35 * z^4)
% ];
% aJ2_E_partial_C = [
...
% ];
aJ2_E_partial_J2 = -3/2*mu/(r^2)*(ae/r)^2*[
    (1 - 5*(z/r)^2)*x/r;
    (1 - 5*(z/r)^2)*y/r;
    (3 - 5*(z/r)^2)*z/r];
aJ2_E_partial_mu = -3/2*J2/(r^2)*(ae/r)^2*[
    (1 - 5*(z/r)^2)*x/r;
    (1 - 5*(z/r)^2)*y/r;
    (3 - 5*(z/r)^2)*z/r];
% aJ2_E_partial_C = [aJ2_E_partial_J2, aJ2_E_partial_mu, zeros(3,n_const)];
...
% ];
amu_Partial_R_N = -mu/r^3*eye(3) + 3*mu/r^5*(r_N*r_N')*eye(3);
% amu_Partial_C = [
%     -(x/r^3), 0, 0;
%     -(y/r^3), 0, 0;
%     -(z/r^3), 0, 0
...
aJ2_N_partial_mu = NE* aJ2_E_partial_mu;
aJ2_N_partial_J2 = NE* aJ2_E_partial_J2;
aJ2_N_partial_C = [aJ2_N_partial_mu, aJ2_N_partial_J2, zeros(3,n_const-2)];
